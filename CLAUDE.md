...
│   ├── node-flow/        #   Node-based flow engine (DAG compile, executor, HMAC webhook trigger)
...
├── mcp/                  # Era-aware MCP wire types shared by client and server (2026-07-28 + legacy)
├── mcp-server/           # MCP server: dispatcher, stdio + Streamable HTTP transports over MCPToolRegistry
