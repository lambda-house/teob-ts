...
describe("modern era", () => {
  ...
  // Spec 2026-07-28: caching hints are MUST on every cacheable complete result.
  // Claude Code rejects a tools/list without them, and the whole server goes
  // dark — connected, authenticated, zero tools.
  it("every cacheable complete result carries ttlMs and cacheScope", async () => {
    for (const method of ["server/discover", "tools/list"]) {
      const res = await server.handle(req(`c-${method}`, method, { _meta: modernMeta }));
      expect(res?.result, method).toMatchObject({ ttlMs: 0, cacheScope: "public" });
    }
  });

  it("tools/call is not cacheable and carries no hints", async () => {
    const res = await server.handle(req(11, "tools/call", { name: "echo", arguments: {}, _meta: modernMeta }));
    const result = res?.result as Record<string, unknown>;
    expect(result["ttlMs"]).toBeUndefined();
    expect(result["cacheScope"]).toBeUndefined();
  });

  it("cacheHints are configurable", async () => {
    const cached = createMCPServer({
      registry: testRegistry(),
      serverInfo: { name: "cached", version: "0" },
      cacheHints: { ttlMs: 300_000, cacheScope: "private" },
    });
    const res = await cached.handle(req(1, "tools/list", { _meta: modernMeta }));
    expect(res?.result).toMatchObject({ ttlMs: 300_000, cacheScope: "private" });
  });

  it("a negative ttlMs fails at construction, not at a real client", () => {
    expect(() =>
      createMCPServer({
        registry: testRegistry(),
        serverInfo: { name: "bad", version: "0" },
        cacheHints: { ttlMs: -1, cacheScope: "public" },
      }),
    ).toThrow(/ttlMs/);
  });
...
describe("legacy era", () => {
  ...
  it("legacy tools/list has NO resultType envelope and no caching hints", async () => {
    const res = await server.handle(req(3, "tools/list"));
    const result = res?.result as Record<string, unknown>;
    expect(result["resultType"]).toBeUndefined();
    // Caching hints arrived with 2026-07-28; a legacy client must not see them.
    expect(result["ttlMs"]).toBeUndefined();
    expect(result["cacheScope"]).toBeUndefined();
