 ...
 *
 * `fast-check` is an optional peer dependency, loaded lazily on the first
 * `invariantProperty`/`assertInvariants` call — consumers that only use the
 * plain AggregateTestKit never need it installed.
 */

import { createRequire } from "node:module";
import type * as fc from "fast-check";

let fcModule: typeof fc | undefined;

function loadFc(): typeof fc {
  if (fcModule === undefined) {
    try {
      fcModule = createRequire(import.meta.url)("fast-check") as typeof fc;
    } catch {
      throw new Error(
        "InvariantTestKit needs the optional peer dependency 'fast-check' for " +
          "invariantProperty/assertInvariants — install it in your project (e.g. `pnpm add -D fast-check`).",
      );
    }
  }
  return fcModule;
