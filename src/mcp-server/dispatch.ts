...
export interface MCPServerOptions {
  ...
  /**
   * Caching hints for cacheable modern results (server/discover, tools/list).
   * Default `{ ttlMs: 0, cacheScope: "public" }`: the tool list is built from
   * the registry alone, so it is identical for every caller and nothing in it
   * varies by authorization — but this server declares no `listChanged`, so it
   * has no way to invalidate a cache it talked a client into keeping. Both
   * results are served from memory; re-fetching is cheap. Raise `ttlMs` when
   * your registry is fixed for the life of the deployment and the round trip
   * is not.
   */
...
export function createMCPServer(opts: MCPServerOptions): MCPServer {
  ...
  const log = opts.log ?? (() => {});
  const cacheHints: CacheHints = opts.cacheHints ?? { ttlMs: 0, cacheScope: "public" };
  ...
  // Spec: servers MUST provide ttlMs >= 0. A negative or non-finite TTL is a
  // programming error in the consumer, so it fails here, not at a real client.
  if (!Number.isFinite(cacheHints.ttlMs) || cacheHints.ttlMs < 0) {
    throw new Error(`MCP cacheHints.ttlMs must be a finite number >= 0, got ${cacheHints.ttlMs}`);
  }
