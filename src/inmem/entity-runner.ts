      ...
      const run = async () => {
        try {
          ...
          if (timeoutMs && onTimeout) {
            // The losing timer must not outlive the race: left uncleared it
            // pins the event loop for the full timeoutMs after every
            // successful sync, keeping short-lived processes from exiting.
            let timer: ReturnType<typeof setTimeout> | undefined;
            try {
              result = await Promise.race([
                effect(),
                new Promise<Either<Failure, Success>>((_, reject) => {
                  timer = setTimeout(() => reject(new Error("__sync_timeout__")), timeoutMs);
                }),
              ]);
            } finally {
              clearTimeout(timer);
